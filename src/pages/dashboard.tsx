...
import { DocumentListPage as ShowCaseList } from 'pages/documents';
...
function DashboardPage() {
  ...
  const documentStat = useSWR<PaginatedInterface<DocumentInterface>>(
    () => `/documents`,
    () => getDocuments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Documents',
      result: documentStat?.data?.totalCount,
      isLoading: documentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`documents`} passHref legacyBehavior>
                  <Button
